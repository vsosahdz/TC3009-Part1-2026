...
# ===========================================================================
# COMPLETA 1 — tu dataset
# ===========================================================================
#
# Esto es todo lo que este archivo sabe de tu problema. Si lo llenas bien, el
# resto del archivo funciona sin que lo abras.
#
# Mientras haya algo vacio, el servicio NO arranca y te dice que falta. Es a
# proposito: un error de configuracion tiene que doler al arrancar, no en la
# primera peticion.
...
# En regresion se ignora, dejalo en None.
CLASE_POSITIVA = None
...
# Las columnas que el modelo va a usar. Las mismas que vas a poner en el
# COMPLETA 2: el tablero y el predictor hablan del mismo vocabulario.
FEATURES = []
...
_vacios = [n for n, v in (("ARCHIVO", ARCHIVO), ("TARGET", TARGET),
                          ("TIPO_TARGET", TIPO_TARGET), ("GRUPO", GRUPO),
                          ("CORTE", CORTE)) if not v]
if not FEATURES:
    _vacios.append("FEATURES")
if TIPO_TARGET == "categoria" and CLASE_POSITIVA is None:
    _vacios.append("CLASE_POSITIVA (es clasificacion: di cual clase te interesa)")
if _vacios:
    raise ValueError(
        "COMPLETA 1: faltan por llenar en backend/s1_tablero.py:\n"
        + "".join(f"    {v}\n" for v in _vacios)
    )
if TIPO_TARGET not in ("numero", "categoria"):
    raise ValueError(
        f"COMPLETA 1: TIPO_TARGET dice {TIPO_TARGET!r} y solo acepta "
        '"numero" o "categoria".'
    )
