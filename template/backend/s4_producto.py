...
def crear_esquema():
    ...
    personales en un sistema real. Piensa que estas guardando.
    """
...
def redactar(entrada, prediccion):
    ...
    Usa las importancias del contrato y compara el caso contra la mediana de
    ...
    # AJUSTA 6 — como se lee tu prediccion.
    #
    # La frase cambia con el problema; la regla no: esta funcion TRADUCE el
    # numero que dio el modelo, nunca lo recalcula.
    etiqueta = contrato.get("target", "el resultado")
    return (
        f"El modelo predice {etiqueta} = {prediccion}. "
        f"Lo que mas pesa en esa decision es {detalle}."
    )
...
def explain():
    ...
    # En regresion la prediccion es un numero; en clasificacion es una clase,
    # que puede ser un entero o una cadena. Forzar float() aqui convertia la
    # clase 0 en "0.0" en la frase, que es un detalle pequeño y se nota mucho.
    if contrato.get("task") != "clasificacion":
        try:
