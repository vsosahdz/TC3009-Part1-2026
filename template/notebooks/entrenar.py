...
# Las features. Tienen que ser las MISMAS que pusiste en el COMPLETA 1, o el
# tablero y el predictor hablarian de cosas distintas.
FEATURES = []

# Cuales de esas features son categoricas. El resto se tratan como numericas.
# Lista vacia si todas son numericas.
CATEGORICAS = []


# ===========================================================================
# COMPLETA 3 — el umbral
# ===========================================================================
#
...
# elegido. Con clases desbalanceadas casi siempre esta mal.
#
# Ponlo, y escribe en tu BITACORA.md POR QUE ese. Eso es lo que se califica.
UMBRAL = None
...
def main():
    vacios = [n for n, v in (("ARCHIVO", ARCHIVO), ("TARGET", TARGET)) if not v]
    if not FEATURES:
        vacios.append("FEATURES")
    if CLASE_POSITIVA is None:
        vacios.append("CLASE_POSITIVA")
    if UMBRAL is None:
        vacios.append("UMBRAL  (COMPLETA 3 — y justificalo en tu bitacora)")
    if vacios:
        raise SystemExit(
    ...
    sobran = [c for c in CATEGORICAS if c not in FEATURES]
    if sobran:
        raise SystemExit(
            f"COMPLETA 2: estas CATEGORICAS no estan en FEATURES: {sobran}"
        )
