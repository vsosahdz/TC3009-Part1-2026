...
**Nunca abras un pull request a este repositorio.** GitHub te ofrece un botón «Compare & pull
request» cada vez que empujas una rama a tu fork, y apunta aquí, no a tu repositorio. Tu
trabajo vive en tu fork y ya está guardado con el `push`. Para traer material del curso se usa
`./setup/run actualizar N`, nunca un pull request. Si lo abres por error, se cierra solo y te
dice dónde mirar.
