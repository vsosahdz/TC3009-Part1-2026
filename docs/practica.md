...
```
   ...
   llenas los cuatro COMPLETA                   entrenar, servidores, pruebas
...
| Llenar los cuatro `COMPLETA` | **computadora**, VS Code | Es código tuyo: tiene que llegar a tu repositorio |
...
│   ├── s2_modelo.py                     predict_proba y el umbral (se lee)
│   └── s4_producto.py                   la explicación (genérica, no se toca)
├── frontend/src/
...
```bash
git remote add curso https://github.com/vsosahdz/TC3009-Part1-2026.git   # si ya lo tienes, da error y no pasa nada
git fetch curso
git checkout -B practica curso/estado/s3
```

Eso crea tu rama de práctica a partir del **estado actual del curso**, que es donde vive
`template/`. Tu `main` no se toca: tu producto de las tres sesiones sigue ahí, y vuelves con
`git checkout main`.

> **Por qué no basta con `git checkout -b practica` a secas.** Tu copia local es de cuando
> hiciste la última sesión, y la práctica se publicó después: `template/` no existe ahí
> todavía. El síntoma es este, y no dice qué pasa:
>
> ```
> zsh: no matches found: template/backend/*
> ```
>
> Traer la rama del curso lo resuelve de una vez, sin depender de cuándo hiciste tu fork.
>
> Y es `checkout -B`, con mayúscula, no `-b`: si ya intentaste una vez y creaste la rama,
> `-b` falla con `fatal: a branch named 'practica' already exists`. `-B` la reposiciona al
> estado correcto en lugar de rendirse.
...
Es el mismo bloque del caso A, precedido del clon.
...
> El remoto del curso sí la tiene siempre.
...
## El boilerplate, ya en tu rama

Los dos casos llegan aquí con `template/` disponible. Cópialo sobre el proyecto y súbelo:
...
```python
ARCHIVO = ""        # el CSV dentro de data/
TARGET = ""         # la columna que vas a predecir
TIPO_TARGET = ""    # "categoria" para clasificación, "numero" para regresión
CLASE_POSITIVA = None   # cuál clase te interesa detectar
GRUPO = ""          # la categórica que agrupa el tablero y hace de filtro
CORTE = ""          # la segunda gráfica, ordinal
FEATURES = []       # las columnas que va a usar el modelo
ID = None           # la que identifica cada fila, si tienes
```

Llega **vacío**, como los esqueletos de cada sesión. Mientras haya algo sin llenar el
servicio no arranca, y te dice exactamente qué falta:

```
COMPLETA 1: faltan por llenar en backend/s1_tablero.py:
    ARCHIVO
    TARGET
    TIPO_TARGET
    GRUPO
...
```
COMPLETA 1: estas columnas no existen en datos.csv: ['colr'].
...
TARGET = ""
CLASE_POSITIVA = None
FEATURES = []       # las MISMAS que en COMPLETA 1
CATEGORICAS = []    # cuáles de esas son categóricas
UMBRAL = None       # ← COMPLETA 3, lee lo de abajo antes de poner nada
```

Mismo trato: si falta algo, `entrenar.py` se detiene antes de leer el CSV.

```
COMPLETA 2: faltan por llenar en notebooks/entrenar.py:
    UMBRAL  (COMPLETA 3 — y justificalo en tu bitacora)
```
...
### `COMPLETA 3` — el umbral, que es la parte que importa
...
`backend/s2_modelo.py` ya viene adaptado a clasificación. **No lo tocas: lo lees.** Fíjate en lo que
hace:
...
- `COMPLETA 4` en `frontend/src/main.jsx` — el título de tu producto

Y ya. La explicación en palabras, el historial y la Model Card salen del contrato: ni una
línea.
...
1. **El servicio no arranca y habla de columnas.** Es `COMPLETA 1`: un nombre mal escrito. El
