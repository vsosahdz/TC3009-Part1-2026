...
> **Tu `artifact_hash` va a ser distinto del que sale aquí, y está bien.** Es la huella de
> *tu* archivo, y los bytes de un modelo serializado cambian entre plataformas —no es el
> mismo Ubuntu que un Mac— aunque el modelo sea idéntico. Lo que sí tiene que coincidir son
> las **métricas**, que son deterministas: mismo `random_state`, mismos números.
>
> El hash sirve para otra cosa: para saber si el artefacto que estás sirviendo es el mismo
> que exportaste. Si cambia sin que hayas reentrenado, algo se movió.
